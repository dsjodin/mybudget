...
export default function Mortgages() {
  ...
  const handleDelete = async (id) => {
    if (confirm('Ta bort detta bolan?')) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Bolan</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        ...
        <Card title="Rantekostnad/man" value={formatSEK(totalInterest)} color="yellow" />
        <Card title="Amortering/man" value={formatSEK(totalAmortization)} color="blue" />
      </div>
          ...
          <div key={loan.id} className="bg-white rounded-xl border p-4">
            <div className="flex items-start justify-between mb-3">
              <div>
                ...
                <p className="text-sm text-gray-500">{loan.lender} — {RATE_TYPES[loan.rate_type]} ranta</p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
              ...
              <div><p className="text-gray-500">Rantekostnad/man</p><p className="font-medium">{formatSEK(loan.monthly_interest_cost)}</p></div>
              <div><p className="text-gray-500">Amortering/man</p><p className="font-medium">{formatSEK(loan.monthly_amortization)}</p></div>
            </div>
          </div>
        ))}
        {loans.length === 0 && (
          <p className="text-center py-12 text-gray-500">Inga bolan registrerade.</p>
        )}
      </div>

      {showForm && (
        <Modal title={editing ? 'Andra bolan' : 'Nytt bolan'} onClose={() => { setShowForm(false); setEditing(null) }}>
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="block text-sm font-medium mb-1">Namn</label>
              <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })}
                className="w-full border rounded-lg px-3 py-2" required placeholder="t.ex. Swedbank 525000" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Rantetyp</label>
                <select value={form.rate_type} onChange={e => setForm({ ...form, rate_type: e.target.value })}
                  className="w-full border rounded-lg px-3 py-2">
                  <option value="variable">Rorlig</option>
      ...
      {showRateModal && (
        <Modal title={`Andra ranta - ${showRateModal.name}`} onClose={() => setShowRateModal(null)}>
          {rateHistory.length > 0 && (
            <div className="mb-4">
              <h3 className="text-sm font-medium mb-2">Rantehistorik</h3>
          ...
          <form onSubmit={handleRateChange} className="space-y-3">
            <div>
              <label className="block text-sm font-medium mb-1">Ny ranta (%)</label>
              <input type="number" step="0.01" value={rateForm.rate}
                onChange={e => setRateForm({ ...rateForm, rate: e.target.value })}
                className="w-full border rounded-lg px-3 py-2" required />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Giltig fran</label>
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button type="submit" className="px-4 py-2 bg-yellow-500 text-white rounded-lg hover:bg-yellow-600">Andra ranta</button>
            </div>
          </form>
        </Modal>
      )}
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-white">
                <tr className="border-b text-left text-gray-600">
                  <th className="py-2 px-2">Man</th>
                  <th className="py-2 px-2 text-right">Ranta</th>
